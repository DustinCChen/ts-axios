...
```bash
git clone https://github.com/alexjoverm/typescript-library-starter.git ts-axios-lib
cd ts-axios-lib
...
## 优秀工具集成
* 使用rollupJS打包
* 使用preitter和eslint格式化代码
* 使用typedoc生成文档
* 使用husky和lint-staged对提交代码进行格式化和校验
* 使用jest 对代码进行单元测试
* 使用coveralls对单元测试覆盖率进行统计
* 使用commitizen对提交信息进行校验
* 使用lerna对子包进行管理
* 使用semantic-release对版本进行管理
* 使用ts-node对代码进行运行
* 使用ts-jest对ts代码进行单元测试
* 使用@types/jest对jest进行类型定义
* 使用@types/node对node进行类型定义

## npm script
* npm run lint:使用tslint工具检查src和test文件夹下代码的可读性、可维护性和功能错误
* npm run lint:fix:使用tslint工具检查src和test文件夹下代码的可读性、可维护性和功能错误，并自动修复
* npm run build:使用rollup打包项目，并且运行typedoc生成文档
* npm run test:使用jest对项目进行单元测试
* npm start:观察者模式运行rollup打包后的代码
* npm run test:watch:使用jest对项目进行单元测试，并开启监听模式
* npm run test:coverage:使用jest对项目进行单元测试，并生成测试覆盖率报告
* npm run docs:使用typedoc生成文档
* npm run docs:serve:使用typedoc生成文档，并开启本地服务器
* npm run docs:json:使用typedoc生成文档，并生成json文件
* npm run commit:运行commitizen，用于格式化的git commit注释

## git 关联远程仓库
* git remote -v
* git remote add origin git@github.com:DustinCChen/ts-axios.git

## 编写基础请求代码1：axios.request(config)
